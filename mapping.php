...
$oncare= $_POST['oncare'];
$siteprefix= $_POST['siteprefix'];
$caliburmfl= $_POST['caliburmfl'];
$asodate='2012-10-31';

foreach($checkbox as $a => $b)
 	{
	//echo ($siteprefix[$checkbox[$a]-1]).'<br/>';
	 if (  $checkbox !=$checkbox[$a] )
     {
	
	$sql=mysql_query('update facility set MFLCode="'.$mflcode[$checkbox[$a]-1].'", distance="'.$distance[$checkbox[$a]-1].'", name="'.$fname[$checkbox[$a]-1].'", type="'.$ftype[$checkbox[$a]-1].'", caliburmfl="'.$caliburmfl[$checkbox[$a]-1].'", siteprefix="'.$siteprefix[$checkbox[$a]-1].'"  where AutoID="'.$fcode[$checkbox[$a]-1].'"') or die(mysql_error());
...
if ($usergroup !=2)
{
//echo '<div class="section-title1">&nbsp</div>';
}
else
{?>
...
if ($usergroup !=2)
{ ?>
<th rowspan=3> # </th><th rowspan=3> County </th><th rowspan=3> District </th> <th rowspan=3> Central site
 </th> <th rowspan=3> Referral sites
 </th><th rowspan=3> Site Code</th><th rowspan=3> Distance* (KM)</th><th rowspan=3> Type </th><th rowspan="3"> Site Prefix</th> <th colspan=2>Type of service</th> 
 <th colspan=4> <div align="center">Total Patients as of <?php echo getmaxasofdatepatientnos(); ?></div>  </th> 
			<th rowspan=3> <div align="center">Equipment</div>  </th> 
		</tr>	
		
			<tr>
	<th rowspan="2"> ART</th>  <th rowspan="2"> PMTCT </th> 
		<th rowspan="2"><center>On Treatment <a href="#" title="Pre-ART">?</a></center>  
			</th><th rowspan="2"> <center> Care <a href="#" title="ART">?</a></center></th><th colspan=2 ><center> Total Patients</center></th>
			
			</tr>	<tr>
		<th> Central</th><th> Referral</th>
...
{?><th rowspan=3> Check </th> 
 ...
 </th><th rowspan=3> Site Code</th><th rowspan=3> Distance* (KM)</th><th rowspan=3> Type </th><th rowspan=3> Site Prefix</th> <th rowspan=3> Calibur Central Site</th><th colspan=2>Type of service</th> 
 <th colspan=3> <div align="center">Total Patients as of <?php echo getmaxasofdatepatientnos(); ?></div>  </th> 
	...
	<th rowspan="2"> ART</th> <th rowspan="2"> PMTCT </th> 
		<th rowspan="2"><center>On Treatment<a href="#" title="Pre-ART">?</a></center>  
			</th><th rowspan="2"> <center> Care <a href="#" title="ART">?</a></center></th><th rowspan="2"><center> Total Patients</center></th>
		...
		<?php }?>	
			<?php
  $sql2=mysql_query("SELECT facility.AutoID as fcode,facility.MFLCode ,facility.name as fname ,facility.district,facility.type,facility.distance,facility.level,facility.centralsiteAutoID,facility.siteprefix from facility,districts where facility.district=districts.ID and districts.county='$countyid' order by facility.AutoID   ") or die(mysql_error());
 	$ffcount;
$totaltests=0;
$totalneedsmets=0;
$i = 0; 
$a=1;
  while(list($fcode,$mflcode,$fname,$district,$type,$distance,$level,$centralsiteAutoID,$siteprefix)=mysql_fetch_array($sql2))
  {$ffcount=$ffcount+1;
...
<tr class="<?php echo $classs; ?>">
			...
			<td><div align="center"><?php echo $typename; ?></div></td>
			<td><div align="center"><?php echo $siteprefix; ?></div></td>

			<?php
			facilityservices($mflcode);
			...
			<td><div align="center"><?php echo patientspercentral($fcode,3); ?>	</div> </td>
			<?php
			} else
			{
			?>
			...
			<?php } ?>
			<td>
			<a href='javascript:void(null);' onClick="popitup('facilityequipments.php?facility=<?php echo $fcode;?>&level=<?php echo $level;?>&centralsiteAutoID=<?php echo 

$centralsiteAutoID ;?>')">View All (<?php echo $totalequipments;?> )</a>  
...
{?>
	  ...
	  ?></td>
			...
			<td><div align="center"><input type="text" name="siteprefix[]" id="siteprefix[]"  size="2" class="text" value="<?php echo $siteprefix; ?>" /></div></td>
			<td><div align="center"><input type="text" name="caliburmfl[]" id="caliburmfl[]"  size="2" class="text" value="<?php echo $caliburmfl; ?>" /></div></td>
	  <?php
	  facilityservices($mflcode);
	  ?>	
