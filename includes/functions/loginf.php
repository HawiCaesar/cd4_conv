...
$loginSql="select username,userID,userGroupID,partnerID,status,partnerID FROM user where username='$username' AND password='$password' LIMIT 1";
     ...
     echo "Username and/or Password incorrect ";
			...
			$_SESSION['log']=systemAccess($loginContent['0'],$loginContent['1'],"program");
			if($loginContent['3']!=0){
...
//resets pssword
	...
	function systemAccess($username,$userID,$level){
	 ...
	 $sql="INSERT INTO accesslog(username,userID,logoutTime,`loginTime`,level) VALUES('$username','$userID','$mydate','$mydate','$level')";		
   ...
   return $mydate;	
	...
	function getUserlog(){
	$sql="SELECT * FROM accesslog ORDER BY `accessID` DESC";	
	$query=mysql_query($sql) or die(mysql_error());
	$myt="";
	$count=1;			
