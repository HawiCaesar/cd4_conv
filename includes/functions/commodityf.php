...
function reagentCategory($mfl) {
  $sql = "SELECT r.categoryID,r.name,r.equipmentType
  				 FROM reagentcategory r,  `equipmentdetails` e 
				 WHERE (r.equipmentType = e.`equipment` OR r.equipmentType=0 ) 
				 	AND e.MFLCode ='$mfl' GROUP BY r.categoryID";
...
<td><input name="beginningbal[' . $rs['reagentID'] . ']" test= "' . $rs['reagentID'] . '" size="5" type="text" class="text" onkeyup="endingBal(' . $rs['reagentID'] . ')" /></td>
<td><input name="receivedqty[' . $rs['reagentID'] . ']" id="receivedqty[' . $rs['reagentID'] . ']" test= "' . $rs['reagentID'] . '" size="5" type="text" class="text" onkeyup="endingBal(' . $rs['reagentID'] . ')" /></td>
<td><input name="receivedlot[' . $rs['reagentID'] . ']" id="receivedlot[' . $rs['reagentID'] . ']" test= "' . $rs['reagentID'] . '" size="5" type="text" class="text" onkeyup="endingBal(' . $rs['reagentID'] . ')" /></td>
<td><input name="qtyused[' . $rs['reagentID'] . ']" id="' . $rs['reagentID'] . '" size="5" test= "' . $rs['reagentID'] . '" type="text" class="text" onkeyup="endingBal(' . $rs['reagentID'] . ')" /></td>
<td><input name="losses[' . $rs['reagentID'] . ']" id="losses[' . $rs['reagentID'] . ']" size="5" test= "' . $rs['reagentID'] . '" type="text" class="text" onkeyup="endingBal(' . $rs['reagentID'] . ')" /></td>
<td><input name="adjustmentplus[' . $rs['reagentID'] . ']" id="adjustmentplus[' . $rs['reagentID'] . ']" test= "' . $rs['reagentID'] . '" size="5" type="text" class="text" onkeyup="endingBal(' . $rs['reagentID'] . ')" /></td>
<td><input name="adjustmentminus[' . $rs['reagentID'] . ']" id="adjustmentminus[' . $rs['reagentID'] . ']" test= "' . $rs['reagentID'] . '" size="5" type="text" class="text" onkeyup="endingBal(' . $rs['reagentID'] . ')" /></td>
...
//Use this fn to disable unavailable equipment
function hasEquip($mfl,$equip) {
	$str="readonly";
  	$sql="SELECT * FROM `facilityequipments`
  				LEFT JOIN `facility`
					ON `facilityequipments`.`facility`=`facility`.`AutoID`
  				LEFT JOIN `equipments`
					ON `facilityequipments`.`equipment`=`equipments`.`ID`
  				WHERE `facilityequipments`.`equipment`=$equip 
				AND `facility`.`MFLCode`=$mfl";
	$query = mysql_query($sql) or die(mysql_error());
	while ($rs = mysql_fetch_array($query)) {
			$str = "";
		}
...
//Number of cd4 equipment in a facility

function equips($mfl,$equip) {
	$str=0;
  	$sql="SELECT count(*) AS equips FROM `facilityequipments`
  				LEFT JOIN `facility`
					ON `facilityequipments`.`facility`=`facility`.`AutoID`
  				LEFT JOIN `equipments`
					ON `facilityequipments`.`equipment`=`equipments`.`ID`
...
function caliburwstablebody($mfl,$filter,$currentmonth,$currentyear,$fromfilter,$tofilter,$fromdate,$todate) {
	 $maxdate=maxdatecommodityfac($mfl);
   	        if ($filter==0) //last submission
	  {
	  	$sql = "SELECT *  FROM  `exp_file_data` where Date_Analyzed='$maxdate'" ;
	    }
	  elseif ($filter==1)//last 6 months $fromdate$todate
	  {
	  $sql = "SELECT *  FROM  `exp_file_data` where Date_Analyzed BETWEEN '$fromdate' AND '$todate' ";
	  	 
	  }
	  elseif ($filter==2)//cusomtize dates $fromfiler $tofilter
	  {
	  $sql = "SELECT *  FROM  `exp_file_data` where Date_Analyzed BETWEEN '$fromfilter' AND '$tofilter' ";	
	    ...
	    elseif ($filter==3)//month/year
	  {
	  	$sql = "SELECT *  FROM  `exp_file_data` where month(Date_Analyzed)='$currentmonth' AND YEAR(Date_Analyzed)='$currentyear' ";	
	  	

 	  }
	    elseif ($filter==4)//year only
	  {
	  		$sql = "SELECT *  FROM  `exp_file_data` where YEAR(Date_Analyzed)='$currentyear' ";	
