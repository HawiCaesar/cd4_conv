...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	submitcommodity($_POST['mfl'], $from, $to, $_POST['caliburtestsAdults'],$_POST['caliburtestsPead'], $_POST['caliburs'], $_POST['counttestsAdults'],$_POST['counttestsPead'], $_POST['counts'], $_POST['cyflowtestsAdults'], $_POST['cyflowtestsPead'],$_POST['cyflows'], $_POST['beginningbal'], $_POST['receivedqty'], $_POST['receivedlot'], $_POST['qtyused'], $_POST['losses'], $_POST['adjustmentplus'], $_POST['adjustmentminus'], $_POST['endbal'], $_POST['requested'], $_POST['namer'],$_POST['comments']);
...
$(document).ready(function() {
		...
		$("#total").val("<?php echo $arr[0]["cyflowtestsAdults"]+$arr[0]["cyflowtestsPead"]+$arr[0]["counttestsAdults"]+$arr[0]["counttestsPead"]+$arr[0]["caliburtestsPead"]+$arr[0]["caliburtestsAdults"];?>");
...
function  validateCommodity(){	
	...
	function printCommodity(){
		...
		?>
		window.location.assign("<?php echo "http://".$_SERVER['SERVER_NAME']."/cd4/cd4commodityprint.php?fcdrrlistID=".$_GET['fcdrrlistID']?>");
		<?php 
				}
		...
		function changeDate(){ 
			...
			if($("#year").val() != ""){		
				...
				startDate.setDate(1);
				stopDate.setYear($("#year").val());
			}
			if(($("#month").val() != "")&&($("#year").val() != "")){
				
				var from_date=year_val+"-"+month_val+"-01";
				$("#from").val(from_date);
			...
			$.ajax({
				...
				async:false,
				data:"DATE="+from_date,
    			url:"<?php echo "http://".$_SERVER['SERVER_NAME']."/cd4/startAndEndDates.php"?>",  
   		 		success:function(data) {   
				var stopDate=data;   				 				
						$("#to").val(stopDate);					
    				}
...
//Calculate Ending Bal
	...
	function clearTempData(){
			...
			$.ajax({
				type:"POST",
				async:false,
				data:"MFLCode="+<?php echo $_SESSION['facility']; ?>,
...
<table class="data-table" >
    ...
    <td colspan="1"><input name="fac" type="input" class="texts" disabled value="<?php echo $_SESSION['username'];?>" size="30"/></td>
    <td><b>Facility Code:</b></td>
    <td><input name="mfl" type="text"  class="texts"  value="<?php echo $_SESSION['facility'];?>" readonly="readonly" disable /></td>
    <td><b>District:</b></td>
    <td><input name="district" type="text"  class="texts" value="<?php echo $rs[14];?>" readonly="readonly" disable /></td>
    <td><b>Province/County:</b></td> 
    <td><input name="county" type="text" class="texts" value="<?php echo  $rs[16];?>" readonly="readonly"  disable /> </td>
    <td><b>Affiliation:</b></td> 
    <td><input name="affiliation" type="text" size="3"  class="texts" value="<?php echo  $rs[8];?>" readonly="readonly" disable /> </td>
  </thead>
  ...
  <tr>
    ...
    <td>
    	<select name="year" id="year" onchange="changeDate()" required >
      <option value=""><i>Select A year</i></option>
      <?php 
	  	$date = date('Y');
		
		for ($n=0; $n<=1;$n++){
			echo "<option value=".((int)$date-$n).">".((int)$date-$n)."</option>";
			
		}
	  ?>
    </select></td>
    <td align="center"><b> Month:</b></td>
    <td><select name="month" id="month" onchange="changeDate()" required >
    ...
    <td colspan="5" >
    	...
    	<input type="text"  name="from" id="from"  placeholder="DD/MM/YYYY" required value="" readonly="readonly"  />
      	<b> to:</b>
        <input type="text" name="to" id="to"  placeholder="DD/MM/YYYY"  required value="" readonly="readonly" /></td>
    	</center>
    </tr>
  <tr>
    <th colspan="1" align="left"><b>State the number of CD4 Tests conducted:-</b></th>
    <td colspan="2"><b> Calibur:</b>
            ...
            <input placeholder="Equip No." name="caliburs"  id="caliburs" class="texts" size="6" required value="<?php echo equips($_SESSION['facility'],2);?>"  readonly /></td>
    <td colspan="3"><b> Count:</b>
            ...
            <input placeholder="Equip No." name="counts" id="counts" class="texts" size="6" required value="<?php echo equips($_SESSION['facility'],1);?>" readonly /></td>
    <td colspan="4"><b>Cyflow Partec:</b> 
            ...
            <input placeholder="Equip No." name="cyflows" id="cyflows" class="texts" size="6" required value="<?php echo equips($_SESSION['facility'],3);?>" readonly />
  ...
  <tr>
    ...
    <td colspan="5"><center><input class="texts" name="total" id="total" type="text" readonly="readonly" value="" /></center></td>
...
<td rowspan="2"><b>COMMODITY NAME</b></td>
            ...
            <td rowspan="2"><b>ENDING BALANCE <br/>PHYSICAL COUNT at end of the Month</b></td>
    ...
    <tbody>  
       ...
       <table class="data-table1" id="table">
			<tr><td colspan="9"><textarea placeholder="FCDRR Comments" name="comments" id="comments" cols="150"></textarea></td></tr>
<tr><td colspan="6"><input type="submit" name="submit" value="Submit Commodity Report" class="button" /><input type="hidden" value="true" name="exists" id="exists" /><input type="hidden" value="true" name="repeat" id="repeat" /> 
       &nbsp;
       <input type="button" value="Reset" class="button" onclick="clearTempData()"/>
       &nbsp;
       <input type="button" onclick="printCommodity()" name="print" value="Print" class="button"  /></td></tr>
</table>
</form>
